...
#? first do `ray start --head`` on head node, and then run this script -- look at ray dashboard to see whether the resources are avalabile
...
NUM_CPUS=$(nproc)
ip_head=$(hostname --ip-address | awk '{print $1}')
#? when running on the head node $SLURM_JOB_NODELIST would be empty, so first we need to salloc
...
# Run training script on all nodes in parallel
